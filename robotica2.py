...
class Frame(object):
    ...
    def __init__(self, dados):
        global x, y, z, u, v, w
        angX, angY, angZ, dx, self.dy, self.dz, self.xB, self.yB, self.zB = dados
        self.angX = np.radians(angX)
        self.angY = np.radians(self.angY)
        self.angZ = np.radians(self.angZ)
        self.r11 = np.cos(self.angZ) * np.cos(self.angY)
        self.r12 = (np.cos(self.angZ) * np.sin(self.angY) * np.sin(angX)) - (np.sin(self.angZ) * np.cos(angX))
        self.r13 = (np.cos(self.angZ) * np.sin(self.angY) * np.cos(angX)) + (np.sin(self.angZ) * np.sin(angX))
        self.r21 = np.sin(self.angZ) * np.cos(self.angY)
        self.r22 = (np.sin(self.angZ) * np.sin(self.angY) * np.sin(angX)) + (np.cos(self.angZ) * np.cos(angX))
